...
// --- CALCUL DE POSITION PRÉCIS (CORRECTIF ANTI-CRASH APPLIQUÉ) ---
const getFleetPosition = (fleet, planets, currentTurn) => {
    ...
    for (let i = 0; i < fleet.path.length - 1; i++) {
        ...
        if (p1 && p2) {
            // ICI : On sécurise avec Number() pour éviter l'erreur SVG "Expected )"
...
// --- SOUS-COMPOSANTS ---
const TopHud = ({ userFaction, factionData, projectedIncome, currentTurn, isAdmin, isProcessingTurn, handleNextTurn, handleLogout }) => (
    <div className="absolute top-4 right-4 z-50 flex flex-col items-end gap-2 pointer-events-none">
        <div className="flex items-start gap-2 pointer-events-auto">
            ...
            <div className="flex items-stretch gap-2 bg-gray-950/90 border border-gray-700 rounded-lg shadow-2xl backdrop-blur-md overflow-hidden p-1">
                ...
                <button onClick={handleLogout} className="px-3 hover:bg-red-900/30 group flex flex-col items-center justify-center transition-colors rounded">
...
export default function GalaxyMap({ userFaction, userRole, userID, userName }) {
  ...
  // --- CALCUL DES PERMISSIONS ET DONNÉES ACTIVES (CORRIGÉ & SÉCURISÉ) ---
  ...
  // --- ACTIONS ÉDITEUR (MODIFIÉES POUR PHRASES DIPLOMATIQUES ET UPDATE) ---
  ...
  // NOUVELLE FONCTION POUR MISE A JOUR FACTION
    ...
    <div className={`flex flex-col h-screen w-screen fixed inset-0 overflow-hidden bg-black select-none font-sans ${movingFleet ? 'cursor-crosshair' : ''}`}>
      ...
      {showSettingsModal && (
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-gray-900 border-2 border-white p-6 z-[100] w-80 shadow-2xl rounded">
              <div className="flex justify-between items-center mb-4 border-b border-gray-700 pb-2"><h3 className="text-white font-bold uppercase">Paramètres Globaux</h3><button onClick={()=>setShowSettingsModal(false)} className="text-gray-400 hover:text-white">✕</button></div>
              <div className="flex justify-between items-center"><span className="text-sm text-gray-300 uppercase">Système de Flotte</span><button onClick={toggleFleetSystem} className={`px-3 py-1 rounded text-xs font-bold uppercase ${isFleetSystemEnabled ? 'bg-green-600 text-white' : 'bg-red-600 text-white'}`}>{isFleetSystemEnabled ? 'ACTIF' : 'INACTIF'}</button></div>
              <p className="text-[10px] text-gray-500 mt-2 italic">Si inactif : Les joueurs ne voient plus les flottes et ne peuvent plus les gérer.</p>
          </div>
      )}

      {/* EDITEUR : GESTION DES FACTIONS (MODIFIÉ POUR PHRASES ET UPDATE) */}
      {showFactionManager && isEditorMode && (
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-gray-900 border-2 border-purple-600 p-6 z-[100] w-[450px] max-h-[80vh] flex flex-col shadow-2xl">
              ...
              <form 
                  ...
                  className="flex flex-col gap-2 mb-4 p-2 bg-black/30 rounded border border-gray-800 overflow-y-auto custom-scrollbar"
              >
      ...
      {/* EDITEUR : ARCHITECTE DE PLANS (BÂTIMENTS) */}
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-gray-900 border-2 border-blue-600 p-6 z-[100] w-[600px] max-h-[90vh] flex flex-col shadow-2xl">
              ...
              <form onSubmit={handleCreateBuilding} className="flex flex-col gap-3 overflow-y-auto custom-scrollbar pr-2 mb-4 bg-black/40 p-4 rounded">
                  <div className="grid grid-cols-2 gap-3">
                      ...
                      {/* NOUVEAU : SÉLECTION DES FACTIONS */}
                      <div className="col-span-2">
                          <label className="text-[10px] text-gray-500 uppercase block mb-1">Factions Autorisées (Vide = Toutes)</label>
                          <div className="flex gap-3 flex-wrap text-[10px] text-gray-300">
                              {factions.map(f => (
                                  <label key={f.id} className="flex items-center gap-1 cursor-pointer">
                                      <input type="checkbox" name="allowed_factions" value={f.id} /> 
      ...
      {/* ÉDITEUR : MODIFICATION DE PROVINCE */}
